-- Recurring occurrences inherit assignee from the template task; template tasks are hidden only while recurrence is active.
...
AS $function$
  ...
  expanded as (
    select
      ...
      coalesce(tt.assignee_id, null::uuid) as assignee_id,
      tp.display_name as assignee_display_name,
    ...
    left join public.tasks tt on tt.id = r.template_task_id
    left join public.profiles tp on tp.user_id = tt.assignee_id
    left join auth.users tu on tu.id = tt.assignee_id
